pipeline {
    ...
    environment {
        ...
        PATH = "/usr/bin:/usr/local/bin:${env.PATH}"  // Ensure docker & kubectl are visible
    }
    parameters {
        booleanParam(
            name: 'ROLLBACK',
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/vibincholayil/todo-app-github-jenkins-aks.git'
            }
        }
...
*/
                ...
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'dockerhub-credentials',
                        ...
                        sh """
                            ...
                            docker tag $IMAGE_NAME ${DOCKER_USERNAME}/todo-app:${BUILD_NUMBER}
                            docker push ${DOCKER_USERNAME}/todo-app:${BUILD_NUMBER}
                        """
                    }
                }
            }
        ...
        stage('Deploy to AKS') {
            steps {
                script {
                    ...
                    // Ensure PVC exists
                    sh """
                    if ! kubectl get pvc todo-pvc -n team-a > /dev/null 2>&1; then
    ...
    post {
        ...
        failure {
            ...
            sh "kubectl rollout undo deployment todo-app -n team-a || true"
        }
    }
}
